...
class SistemYoneticisiEdit extends Component {
    ...
    async componentDidMount() {
        if (this.props.match.params.id !== 'new') {
            const sistemyonetici = await(await (await fetch(`/sistemyoneticisi/getir?id=${this.props.match.params.id}`)).json()).resultValue;
           fetch(`/sistemyoneticisi/getir?id=${this.props.match.params.id}`)
             .then(response => response.json())
             .then(data => this.setState({item: data.resultValue}));
             this.setState({item: sistemyonetici});
        }
    }
...
async handleSubmit(event) {
    ...
    await fetch('/sistemyoneticisi' + (item.id ? '/guncelle/' + item.id : '/ekle'), {
    ...
    this.props.history.push('/sistemyoneticisi');
}

    render() {
        const {item} = this.state;
        const title = <h2>{item.id ? 'Sistem Yöneticisi Güncelle' : 'Sistem Yöneticisi Ekle'}</h2>;
            ...
            <Container>
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    <FormGroup>
                        ...
                        <Button color="secondary" tag={Link} to="/sistemyoneticisi">Cancel</Button>
                    </FormGroup>
                </Form>
            </Container>
        </div>
...
export default withRouter(SistemYoneticisiEdit);