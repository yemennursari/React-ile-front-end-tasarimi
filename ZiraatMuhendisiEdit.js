...
class ZiraatMuhendisiEdit extends Component {
    ...
    async componentDidMount() {
        if (this.props.match.params.id !== 'new') {
            const ziraatmuhendisi = await(await (await fetch(`/ziraatmuhendisi/getir?id=${this.props.match.params.id}`)).json()).resultValue;
           fetch(`/ziraatmuhendisi/getir?id=${this.props.match.params.id}`)
             .then(response => response.json())
             .then(data => this.setState({item: data.resultValue}));
             this.setState({item: ziraatmuhendisi});
        }
    }
...
async handleSubmit(event) {
    ...
    await fetch('/ziraatmuhendisi' + (item.id ? '/guncelle/' + item.id : '/ekle'), {
    ...
    this.props.history.push('/ziraatmuhendisi');
}

    render() {
        const {item} = this.state;
        const title = <h2>{item.id ? 'Ziraat Mühendisi Güncelle' : 'Ziraat Mühendisi Ekle'}</h2>;
        ...
        return <div>
            ...
            <Container>
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    <FormGroup>
                        <Label for="uzmanlikalani">uzmanlikalani</Label>
                        <Input type="text" name="uzmanlikalani" id="uzmanlikalani" value={item.uzmanlikalani || ''}
                               onChange={this.handleChange} autoComplete="uzmanlikalani"/>
                    </FormGroup>  
                    ...
                    <FormGroup>
                        ...
                        <Button color="secondary" tag={Link} to="/ziraatmuhendisi">Cancel</Button>
                    </FormGroup>
                </Form>
            </Container>
        </div>
...
export default withRouter(ZiraatMuhendisiEdit);
